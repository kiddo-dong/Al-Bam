...
- **재기동마다 데이터를 다시 채우는 배치/적재 로직**: 대상이 인메모리면 무해하지만, 영속 저장소(pgvector 등)에서는 중복이 쌓인다. 근로기준법 Q&A 지식베이스가 이 문제를 겪어 기동 시 자동 적재를 없애고 관리자 트리거 API로 바꿨으며, 청크 ID를 내용 해시로 고정해 재적재 시 덮어쓰게 했다. 비슷한 적재 로직을 추가할 때 같은 방식을 따를 것.
